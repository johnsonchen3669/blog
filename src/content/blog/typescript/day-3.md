...
> 負責建立並回傳物件或函式的函式，常稱為工廠函式。`createAnswerRecorder` 每次執行都會建立一個新的作答紀錄函式。
...
- 閉包讓函式持續存取宣告位置的外部變數。當內層函式被回傳時，即使外層函式已經執行完畢，仍能繼續累計作答次數或記住題號。

下一篇會看 Prototype、Class 與 `this`，理解 JavaScript 如何查找物件方法，以及 `this` 為什麼會隨呼叫方式改變。
